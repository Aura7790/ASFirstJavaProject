...
public class LogicalOperations {
    ...
    public static String myGreeting(String greeting, String myName){
        return greeting + "\n" + myName;
    }
    ...
    // a. -5 + 8 * 6
    public static int operationA(int first, int second, int third){
        return first + second * third;
    }
    // b. (55+9) % 9
    public static int operationB(int first, int second) {
        return (first + second) % second;
    }

    //c. 20 + -3*5 / 8
    public static float operationC(int first, int second, int third, int fourth) {
        return first + (float) (second * third) / fourth;
    }

    //d. 5 + 15 / 3 * 2 - 8 % 3
    public static int operationD(int j, int k, int l, int m, int n, int o) {
        return j + k / l * m - n % o;
    }
}